...
const slides: { title: string; body: string; icon: SFSymbolName; tint: string; demo: string; tag?: string }[] = [
  ...
  { title: "Keep what speaks to you", body: "Tap a verse to highlight it or add a personal note. Hold a verse to select several, then choose a color or add a note.", icon: "pencil.tip", tint: "#3D3320", demo: "A thought worth keeping." },
  ...
  { title: "Discover Bible Moments", body: "Look for a softly glowing verse. Tap it to reveal a special moment and learn why it matters. Try Genesis 1:1 or Matthew 28:6.", icon: "sun.max", tint: "#3D2D24", demo: "In the beginning…" },
];
          ...
          renderItem={({ item, index: slideIndex }) => <View accessibilityElementsHidden={slideIndex !== index} importantForAccessibility={slideIndex === index ? "auto" : "no-hide-descendants"} style={{ width: pageWidth }}>
            <ScrollView contentContainerStyle={{ paddingBottom: 20 }}>
              ...
              <View style={{ paddingHorizontal: 26, paddingTop: 24 }}>
                ...
                <Text accessibilityRole="header" style={[systemText.title2, { color: "#FFFFFF", textAlign: "center", marginBottom: 14 }]}>{item.title}</Text>
                <Text style={[systemText.callout, { lineHeight: 24, color: "#C4C4CA", textAlign: "center" }]}>{item.body}</Text>
              </View>
            </ScrollView>
          </View>} />
